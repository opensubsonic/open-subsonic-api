...
Reports playback timeline state for a song. Clients should call the endpoint at least on each state change. The end point can be called every X seconds by clients, but servers should estimate current position based on playbackRate for up to date data between calls. Servers should not instantly assume end of playback after the calculated time passes the duration, as the client could be late or not properly implement playbackRate, so should have a larger grace period.
