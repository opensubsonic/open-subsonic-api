...
| `role`  | `string`                      | No      | **Yes** | Semantic role of the cues in this cueLine. Omitted or empty for main vocals. Values: `bg` (background vocals), `voice1` through `voiceN` (individual voice parts — N is any positive integer with no upper bound, e.g. `voice1`, `voice100`, `voice1000`), `group` (group/chorus vocals). When multiple cueLines share the same `index`, the main vocals cueLine **must** come first |
